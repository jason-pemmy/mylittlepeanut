...
// register_globals=Off mod by http://www.dreamscape.biz
//import_request_variables("gpc");

// duplicate To: header mod by http://www.dreamscape.biz

// for ultimate security, use this instead of using the form
//$recipient = " 	gbrotary@gmail.com"; // youremail@domain.com
$recipient = "jason.pemmy@gmail.com"; // youremail@domain.com
// bcc emails (separate multiples with commas (,))
$bcc = "jason.pemmy@gmail.com";

// referers.. domains/ips that you will allow forms to
// reside on.
$referers = array ('mylittlepeanut.ca');
...
// send it off
mail_it(stripslashes($content), ($subject)?stripslashes($subject):"mylittlepeanut.ca Web Form Submission", $email, $recipient);
if (file_exists($ar_file)) {
   $fd = fopen($ar_file, "rb");
   $ar_message = fread($fd, filesize($ar_file));
   fclose($fd);
   mail_it($ar_message, ($ar_subject)?stripslashes($ar_subject):"mylittlepeanut.ca Web Form Submission", ($ar_from)?$ar_from:$recipient, $email);
}

// if the redirect option is set: redirect them
if (!$redirect) {
   header("Location: http://mylittlepeanut.ca/#contact?success=true");
   exit;
} else {
