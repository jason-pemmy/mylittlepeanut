$(document).ready(function(){
	...
	window.sleepsacks = [{	"id":"ss0",
					...
					{	"id":"ss6",
						...
						"details":"<p><b>Soar into the clouds on a hot air balloon ride with a panda</b></p>Soft and fluffy fleece<br/>80% organic cotton 20% bamboo charcoal"}];
	...
	window.loadInventory = function(items){	
		...
		for(var i=0;i<items.length;i++){
			...
			data += "<div class='item-display'><a class='various' href='#"+items[i].id+"'>";
			...
			data += "<div id='"+items[i].id+"' style='display:none'>";
			data += "<div class='litebox-image-container'>";
			data += "<img src='images/inventory/"+items[i].image+"' width='100%'/></div>";
			data += "<div style='clear:both;float:left'>"+items[i].details+"</div>"
			data += "<div id='item-title'style=''>";
			data +=  ""+items[i].title+"</div>";
			data += "<div id='item-price' style=''>"+items[i].price+"</div>";
			data += "<div id='litebox-container' style=''>";
			data += "<div id='litebox-cartbtn'><div class='buyit-btn' data-button_background_color='593d36' data-button_text_color='ffffff' data-buy_button_text='Add to cart' data-display_size='compact' data-embed_type='product' data-has_image='false' data-product_handle='"+items[i].shopifyID+"' data-product_name='"+items[i].title+"' data-redirect_to='cart' data-shop='my-little-peanut-2015.myshopify.com'></div></div>";
			data += "</div></div></div>";			
			data += "";
		}
		ic.html(data);	
